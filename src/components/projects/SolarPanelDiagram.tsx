...
import { X, Cpu, Network, Database, Layout, Code as CodeIcon } from 'lucide-react';
import { cn } from '@/lib/utils';
import { initialNodes, initialEdges, type SolarPanelNode } from '@/data/solar-panel-nodes';
...
const SensorNode = ({ data, selected }: NodeProps<SolarPanelNode>) => (
    <BaseNode label={data.label} icon={Cpu} selected={selected} type="sensor">
        <Handle type="source" position={Position.Right} className="!bg-emerald-500" />
    </BaseNode>
);

const BrokerNode = ({ data, selected }: NodeProps<SolarPanelNode>) => (
    <BaseNode label={data.label} icon={Network} selected={selected} type="broker">
        <Handle type="target" position={Position.Left} className="!bg-blue-500" />
        <Handle type="source" position={Position.Right} className="!bg-blue-500" />
    </BaseNode>
);

const BackendNode = ({ data, selected }: NodeProps<SolarPanelNode>) => (
    <BaseNode label={data.label} icon={Database} selected={selected} type="backend">
        <Handle type="target" position={Position.Left} className="!bg-purple-500" />
        <Handle type="source" position={Position.Right} className="!bg-purple-500" />
    </BaseNode>
);

const DashboardNode = ({ data, selected }: NodeProps<SolarPanelNode>) => (
...
export default function SolarPanelDiagram() {
    ...
    const [selectedNode, setSelectedNode] = useState<SolarPanelNode | null>(null);

    const onNodeClick = useCallback((_: React.MouseEvent, node: SolarPanelNode) => {
        ...
        <div className="relative h-[600px] w-full rounded-xl border border-border bg-zinc-950/50 backdrop-blur-sm overflow-hidden">
            ...
            >
                ...
                <Panel position="top-left" className="bg-popover/80 p-2 rounded-md border border-border backdrop-blur-md">
                    <h3 className="text-xs font-bold font-mono text-muted-foreground uppercase">IoT Architecture</h3>
                </Panel>
            </ReactFlow>
                ...
                {selectedNode && (
                    ...
                    >
                        ...
                        <div className="absolute bottom-6 left-6 right-6">
                            <button className="flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-xs font-bold text-primary-foreground hover:opacity-90 transition-all">
                                Learn Integration
                            </button>
                        </div>
                    </motion.div>
